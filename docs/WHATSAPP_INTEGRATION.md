...
- **`evolution.ts`** — Evolution API (gateway não-oficial sobre WhatsApp Web).
  É o **default** e funciona em modo mock quando não há env configurada.
- **`meta.ts`** — WhatsApp Cloud API (Meta Business). É um **stub tipado**: os
  métodos lançam `Error("não implementada")`. Existe para garantir que a migração
  futura não exija mudar quem chama o adapter.

A escolha é feita em `lib/whatsapp/provider.ts` (`WHATSAPP_PROVIDER=meta` força o
stub Meta; caso contrário, Evolution).
...
2. Normaliza o payload com `adapter.parseWebhook` → `InboundMessage`.
3. Roda a **triagem** (`triage-bot.ts`).
4. Faz upsert da conversa e persiste a mensagem recebida.

### Envio (rota)

`app/api/whatsapp/send/route.ts` exige `can(principal, "whatsapp", "create")`,
envia via adapter (texto ou template) e persiste a mensagem enviada.
...
3. **Implementar `meta.ts`**: preencher os métodos do stub usando o Graph API
   (`/messages`), mapeando `templateKey` → nome aprovado e `vars` → componentes.
...
> Começar com Evolution API (rápido, barato, com mitigações de banimento), tratar
> isso como dívida consciente e migrar para a Meta Cloud API trocando apenas o
